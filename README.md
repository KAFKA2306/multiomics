...
公開ページは、保存済みの正準データを読み込み、承認・臨床試験・シーケンシング費用・FDAデータベース間のcoverageを同じ「最新値」にまとめず、観測時点と一次情報を分けて表示します。データ取得に失敗した場合は古い値へfallbackしません。
...
- FDA product coverage analysis: [`api/v1/multiomics/fda-product-coverage-analysis.json`](api/v1/multiomics/fda-product-coverage-analysis.json)
- FDA product coverage evidence: [`api/v1/multiomics/fda-product-coverage-evidence.json`](api/v1/multiomics/fda-product-coverage-evidence.json)
...
- Orange Book Additions/Deletions: https://www.fda.gov/drugs/drug-approvals-and-databases/additionsdeletions-prescription-and-otc-drug-product-lists
