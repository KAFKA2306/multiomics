...
class _MainTextParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        if tag == "main":
            self._in_main = True
        elif self._in_main and tag in {"script", "style"}:
    ...
    def handle_endtag(self, tag: str) -> None:
        if tag == "main":
            self._in_main = False
            self._ignored_depth = 0
        elif self._in_main and tag in {"script", "style"} and self._ignored_depth:
            self._ignored_depth -= 1

    def handle_data(self, data: str) -> None:
        if self._in_main and not self._ignored_depth:
